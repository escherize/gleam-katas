...
pub fn new(id: ShipmentId, order_id: OrderId) -> Shipment {
  Shipment(id, order_id, Pending)
}

pub fn id(s: Shipment) -> ShipmentId { s.id }
pub fn order_id(s: Shipment) -> OrderId { s.order_id }

pub fn mark_shipped(s: Shipment) -> Result(Shipment, ShipmentError) {
  case s.status {
    Pending -> Ok(Shipment(..s, status: Shipped))
...
pub fn mark_delivered(s: Shipment) -> Result(Shipment, ShipmentError) {
  case s.status {
    Shipped -> Ok(Shipment(..s, status: Delivered))
...
```gleam
// src/shipping/handle_order_placed.gleam
import gleam/result
import order.{type OrderEvent, OrderPlaced}
...
pub fn run(
  ...
  case event {
    OrderPlaced(order_id, _total) -> {
      // Idempotency: don't create twice for the same order
      case repo.find_by_order(order_id) {
        Ok(_) -> Ok(Nil)
        Error(NotFound) -> {
          let s = shipment.new(fresh_id, order_id)
          repo.save(s) |> result.map_error(RepoFailed)
        }
      }
    }
    _ -> Ok(Nil)  // ignore other event variants
  }
}
```

(The repo implementation is structurally identical to
`order_repo.in_memory` from kata 6, with one extra message variant
for `find_by_order`. Skipping it here for length — write it the same
way.)
